...
exports.imageSearchService = exports.ImageSearchService = void 0;
...
class ImageSearchService {
    ...
    async searchByKeyword(keyword, filters) {
        try {
            const params = { q: keyword };
            if (filters?.mediaType)
                params.media_type = filters.mediaType;
            if (filters?.yearStart)
                params.year_start = filters.yearStart.toString();
            if (filters?.yearEnd)
                params.year_end = filters.yearEnd.toString();
            if (filters?.center)
                params.center = filters.center;
            if (filters?.photographer)
                params.photographer = filters.photographer;
            if (filters?.location)
    ...
    async searchByCenter(center, filters) {
        try {
            const params = { center };
            if (filters?.mediaType)
                params.media_type = filters.mediaType;
    ...
    async searchByPhotographer(photographer, filters) {
        try {
            const params = { photographer };
            if (filters?.mediaType)
                params.media_type = filters.mediaType;
            if (filters?.yearStart)
                params.year_start = filters.yearStart.toString();
    ...
    async searchByYearRange(yearStart, yearEnd, filters) {
        try {
            const params = {
    ...
    async searchByMediaType(mediaType, filters) {
        try {
    ...
    validateImageSearchParams(params) {
        const errors = [];
...
exports.imageSearchService = new ImageSearchService();
